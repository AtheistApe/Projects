...
VALID_EFFECTS = ('enlarge-font-on-focus',
                 'raise-line-padding-on-focus', 'raise-col-padding-on-focus')
...
class KezMenuEffectAble(object):
    ...
    def enableEffect(self, name, **kwargs):
        ...
        self.__getattribute__(
            '_effectinit_%s' % name.replace("-", "_"))(name, **kwargs)
    ...
    def _updateEffects(self, time_passed):
        ...
        for name, params in self._effects.items():
            self.__getattribute__(
                '_effectupdate_%s' % name.replace("-", "_"))(time_passed)
    ...
    def _effectinit_enlarge_font_on_focus(self, name, **kwargs):
        ...
        kwargs['raise_font_ps'] = kwargs['enlarge_factor'] / \
            kwargs['enlarge_time']  # pixel-per-second
    ...
    def _effectinit_raise_line_padding_on_focus(self, name, **kwargs):
        ...
        if not kwargs.has_key('padding'):
            kwargs['padding'] = 10
        kwargs['padding_pps'] = kwargs['padding'] / \
            kwargs['enlarge_time']  # pixel-per-second
