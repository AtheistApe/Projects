...
class Main():
    ...
    def place_monster(self):
        self.monster_position = [
            random.randint(0, self.max_width - 1), random.randint(0, self.max_height - 1)]
    ...
    def display_menu(self):
        menu_list = ['Start New Game', '[Save Game]',
                     '[Load Game]', 'Customize Setup', 'Exit']
    ...
    def check_boundary(self, new_x, new_y):
        min_width = 0
        min_height = 0
        if(new_x < max_width or new_x == self.max_width or new_y < min_height or new_y == self.max_height):
            return False
        else:
            return True
    ...
    def player_move(self, choice):
        ...
        if(choice == 'W' or choice == 'w'):
            if(self.check_boundary(current_x, current_y - 1)):
                return False
            else:
                self.character_position = [current_x, current_y - 1]
                return True
        elif(choice == 'A' or choice == 'a'):
            if(self.check_boundary(current_x - 1, current_y)):
                return False
            else:
