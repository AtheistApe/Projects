...
for student in students_list:
    ...
    stDict = {'last_name':last_name, 'first_name':first_name,
...
for course in courses:
    outPutFileName = course + '.csv'
    fileObj = open("./" + outPutFileName, "w")
    for student in wwClass:
        if student['course'] == course:
            nameStr = '"' + student['last_name'] + ', ' + student['first_name'] + '"'
            addrStr = '<' + student['email'] + '>'
            stStr = nameStr + ' ' + addrStr + '\n'
            fileObj.write(stStr)
    fileObj.close()
